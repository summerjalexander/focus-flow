...
import { PauseIcon, PlayIcon, RefreshCwIcon, BrainIcon, ChevronUpIcon, ChevronDownIcon } from './Icons.tsx';
    ...
    <div className="my-6 p-6 bg-[#007370]/10 rounded-xl border border-[#69adaf] text-center">
      ...
      <div className="relative w-48 h-48 mx-auto flex items-center justify-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="font-mono text-5xl text-[#f7f7f7] my-1">{formatTime(session.timeLeft)}</span>
      ...
      <div className="flex justify-center items-center space-x-6 mt-6">
        <button onClick={onReset} className="p-3 rounded-full bg-[#69adaf]/20 hover:bg-[#69adaf]/40 transition-colors" aria-label="Reset timer">
            <RefreshCwIcon className="w-6 h-6 text-[#f7f7f7]" />
        </button>
        <button onClick={onPause} className="w-20 h-20 flex items-center justify-center rounded-full bg-gradient-to-br from-[#ee6650] to-[#d95a46] text-[#f7f7f7] shadow-lg shadow-[#ee6650]/30 transform hover:scale-105 transition-transform" aria-label={session.isActive ? 'Pause timer' : 'Resume timer'}>
          {session.isActive ? <PauseIcon className="w-10 h-10" /> : <PlayIcon className="w-10 h-10 pl-1" />}
        </button>
      ...
      {session.mode === 'work' && (
        <div className="mt-8 border-t border-[#69adaf]/30 pt-6">
            <form onSubmit={handleLogSubmit}>
                <label htmlFor="distraction-input" className="flex items-center justify-center text-md font-semibold text-[#69adaf] mb-3">
