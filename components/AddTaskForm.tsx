...
const AddTaskForm: React.FC<AddTaskFormProps> = ({ onAddTask, taskCount, categories }) => {
  ...
  const isDisabled = taskCount >= MAX_TASKS;
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
            <div className="flex items-center space-x-2">
                <input
                    ...
                    placeholder={isDisabled ? "5 tasks is the limit!" : "Add a new priority..."}
                    disabled={isDisabled}
                    className="flex-grow bg-transparent border-2 border-[#69adaf] rounded-lg py-3 px-4 text-[#f7f7f7] placeholder-[#69adaf]/70 focus:outline-none focus:ring-2 focus:ring-[#ee6650] focus:border-[#ee6650] transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <button
                    type="submit"
         ...
         {isDisabled && <p className="text-center text-sm text-[#ee6650] mt-2">Focus on your top 5! You can add more tomorrow.</p>}
    </div>
  );
};
